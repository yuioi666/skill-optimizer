...
3. 把下面这段提示词发给 Codex，并补充你的具体要求：

   > 请根据我的需求评估这个 Skill。它应该……，不能……。先整理验收标准并设计开发集、回归集和保留集。在产生模型调用前，请列出三种可选线路及预计调用量，让我选择：无 Skill 与原版对比，用于验证 Skill 是否真的有效；原版与候选对比，用于优化并防止退步；三者全部对比，用于同时验证有效性和优化效果。请创建多个 Agent，让我能看到需求分析、测试设计、评估执行和结果审计的过程。不要覆盖原始 Skill，候选版本只有通过验收后才能交付。

   如果已经确定目的，也可以直接指定线路：

   - 只验证有效性：`请用“无 Skill vs 原版”线路测试这个 Skill 是否有效，不要优化。`
   - 只进行优化：`请用“原版 vs 候选”线路优化这个 Skill，并检查是否出现退步。`
   - 完整评估：`请用“三者全部”线路，比较无 Skill、原版和优化候选。`
